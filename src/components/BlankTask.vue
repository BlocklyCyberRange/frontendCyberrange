        ...
        <div class="columns is-hcentered mt-5">
          ...
          <div class="ml-4 is-hcentered">
            ...
            >
              ...
              >
                <button
                    @click="viewJson=false; showTask=true;"
                ...
                <button
                    @click="viewJson=true; showTask=true"
                    class="button is-rounded is-light"
                    :class="{'is-red-br':viewJson && showTask}"
                >
                  Json
      ...
      <div v-if="showTask">
        ...
        >
          <div :id="'blocklyGroupContainer' + taskData.blockly_task">
            ...
            <div class="submitContainer" v-if="!noTriesLeftBlockly && !completedBefore">
              <ul>
                <li class="submitItem" v-for="(item, index) in blanks" :key="item">
                  <span class="blockly-input">
                    ...
                    Your current input: "<span class="duplicate-content" :id="'watcher-task-' + taskData.blockly_task + '-' + taskData.blockly_block_order[index]">...</span>"
                  </span>

                  <br>

                  <span v-if="!blanks[index].rightTry && !noTriesLeftBlockly && !completedBefore" class="blockly-submit">
                    Submit your input: <button
                        @click="validateInputBlockly(index)/*this.$refs.blankComponent.validateInput(true, index)*/"
                  ...
                  <span
                    class="blockly-hint"
                    v-if="!blanks[index].rightTry && !noTriesLeftBlockly && !completedBefore && !hintActivatedBlockly[index] && blanks[index].dataTooltip != null"
                  >
                    Get a hint: <span
                  ...
                  <text v-if="hintActivatedBlockly[index] && blanks[index].dataTooltip != null" class="blockly-hint has-text-primary">
                    Hint: {{blanks[index].dataTooltip}} (-1 point)
                  </text>

                  <span class="blockly-message">
                    <span v-if="emptyInputBlockly[index]" class="has-text-danger" >Input cannot be empty.</span>
                    <span v-else-if="triesLeftBlockly[index] < 3 && triesLeftBlockly[index] > 0 && !completedBefore && !blanks[index].rightTry" class="has-text-danger" :id="'error-message-' + taskData.blockly_task + '-' + index">
                      You were wrong. You have {{ triesLeftBlockly[index] }} Tries left.
                    </span>
                    <span v-else-if="blanks[index].rightTry" class="has-text-primary" :id="'success-message-' + taskData.blockly_task + '-' + index">
                      Great Try! You earned {{ triesLeftBlockly[index] }} point(s).
                    </span>
                  </span>
            ...
            <div class="no-tries-left has-text-danger" v-else-if="noTriesLeftBlockly">
          ...
          <div class="directive-header">
            ...
            <div
                ...
                :key="rule"
                :class="{'is-rule-level-2': index==1, 'is-rule-level-3': index==2}"
            >
...
export default {
  ...
  methods: {
    ...
    validateInputBlockly(index) {
      console.log("validateInputBlockly: "+index+" --- "+"blockly_task: "+this.taskData.blockly_task);

      var emptyXML = document.getElementById("workspaceBlocks_task"+this.taskData.blockly_task+"_empty"+index);
      var solutionXML = document.getElementById("workspaceBlocks_task"+this.taskData.blockly_task+"_solution"+index);
      ...
      tool.diffAsXml(emptyXML_string1, workspaceXML_string, schema, options, (result) => {
        console.log(result);
        if(result.length === 0) {
      ...
      tool.diffAsXml(emptyXML_string2, workspaceXML_string, schema, options, (result) => {
        console.log(result);
        if(result.length === 0) {
      ...
      tool.diffAsXml(emptyXML_string3, workspaceXML_string, schema, options, (result) => {
        console.log(result);
        if(result.length === 0) {
      ...
      if(xml_is_empty1 || xml_is_empty2 || xml_is_empty3) {
        ...
        try {
          ...
          allTries[this.taskData.tileNo][index] = allTries[this.taskData.tileNo][index]-1;
    ...
    completedBlockly(index) {
      return !(this.triesLeftBlockly[index] > 0 && !this.blanks[index].rightTry);
    },
    getTriesLeftBlockly() {
      if(localStorage.getItem("storedData")!= null){
        console.log("Tries from localStorage");
        var triesFromLocalStorage = JSON.parse(localStorage.getItem("storedData"))[this.taskData.tileNo];
    ...
    display_blockly_task() {
      ...
      if(this.$parent.showBlockly) {
        ...
        var jsonContainerHide = document.getElementById("jsonGroupContainer"+task);
        ...
        if(!this.completedBefore) {
          ...
          var blocklyEditableText = document.getElementsByClassName("blocklyEditableText");
          for(var i = 0; i < blocklyEditableText.length; i++) {
            var blocklyItem = blocklyEditableText.item(i).getElementsByClassName('blocklyText')[0];
